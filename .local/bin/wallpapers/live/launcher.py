...
def check_cache_file():
    pathlib.Path(CACHE_THEME.parent).mkdir(parents=True, exist_ok=True)
    if not pathlib.Path(CACHE_THEME).exists():
        print("not have system theme CREATE !!!!")
        with open(CACHE_THEME, "w") as f:
            f.write("catppuccin")
    else:
        print(f"Cache theme {CACHE_THEME} exists, nothing to do.")
        sys.exit(0)
